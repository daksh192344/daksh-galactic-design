...
const AdminDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      <aside className={`fixed md:static z-50 h-screen w-64 glass-card border-r border-border/30 flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        <div className="p-5 border-b border-border/30 flex items-center justify-between">
          <a href="/" className="font-display text-lg font-bold"><span className="neon-text">Daksh</span>.dev</a>
        ...
        <nav className="flex-1 p-3 space-y-1">
          {tabs.map((tab) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-body transition-all ${activeTab === tab.id ? "neon-button" : "text-muted-foreground hover:text-foreground hover:bg-secondary/50"}`}
            >
              <tab.icon size={18} />
              {tab.label}
            </button>
        ...
        <div className="p-3 border-t border-border/30">
          <button onClick={signOut} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-body text-muted-foreground hover:text-destructive transition-colors">
      ...
      <main className="flex-1 flex flex-col min-h-screen overflow-hidden">
        <header className="h-14 border-b border-border/30 flex items-center px-4 gap-4 glass-card">
          <button className="md:hidden text-foreground" onClick={() => setSidebarOpen(true)}><Menu size={20} /></button>
          <h2 className="font-display text-sm font-semibold capitalize">{tabs.find((t) => t.id === activeTab)?.label}</h2>
        </header>
